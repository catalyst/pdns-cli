...
domains="$(./pdns -c conf.toml show-rrsets $zone | awk '{ print $1" "$4" "$5}')"
if [ ! -z "${filter}" ]; then
...
while read -r line; do
    echo "./pdns -c conf.toml edit-rrset --ttl $ttl --replace $zone $line";
    if [ ! -z "${do}" ]; then
        ./pdns -c conf.toml edit-rrset --ttl $ttl --replace $zone $line
    fi;
done <<< "$domains"
exit ${rc}
